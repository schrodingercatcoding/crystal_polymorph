...
from generate_new_frame_update_trail3 import generate_translate_frame, \
...
def Monte_Carlo(atoms, calculator, space_group, step_size, perturbation, counter, numb_molecules, molecule1_in_cell=[], MC_steps = 1000, debug=False):
    ...
    if new_E < old_E:
        ...
        if p > nu:
            ...
            atoms.set_cell(old_cell)
            atoms.set_positions(old_xyz)
    with open('MC_trace.txt', 'a') as fh:
        fh.write('%f  %d'%(new_E, counter))
