...
def generate_rotation_frame(atoms, molecule1_atoms_index, space_group, sigma=0.1):
    ...
    atoms.set_positions(np.array(new_XYZ_to_set))
...
def generate_cell_length_a_frame(atoms, molecule1_atoms_index, space_group, sigma=0.1):
    ...
    molecule1_vectors = np.array([old_positions[i] for i in molecule1_atoms_index])
    ...
    atoms.set_cell([new_a, new_b, new_c, alpha, beta, gamma], scale_atoms=True)
    ...
    new_molecule1_vectors = get_xyz_after_move(old_xyz, new_xyz, molecule1_vectors)
...
if __name__ == "__main__": 
    ...
    generate_cell_angle_beta_frame(atoms, molecule1_atoms_index, space_group, 1.0) 
    ret_atoms = atoms.copy()

    # This is used to randomly generate a large perturbation
    ...
    molecule1_atoms_index = molecule_lists(ret_atoms)[0] 
